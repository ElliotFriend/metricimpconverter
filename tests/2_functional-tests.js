  ...
  suite('Integration tests with `chai-http`', function() {
    ...
    test('Convert a valid input such as 10L: GET request to /api/convert.', (done) => {
      chai
        ...
        .end( (err, res) => {
          ...
          assert.approximately(res.body.returnNum, 2.64172, 0.1, 'returnNum should be "2.64172"')
    ...
    test('Convert an invalid input such as 32g: GET request to /api/convert.', (done) => {
      chai
        ...
        .end( (err, res) => {
          assert.equal(res.body, 'invalid unit', 'returned text should be "invalid unit"')
          done()
        })
    })
    ...
    test('Convert an invalid number such as 3/7.2/4kg: GET request to /api/convert.', (done) => {
      chai
        ...
        .end( (err, res) => {
          assert.equal(res.body, 'invalid number', 'returned text should be "invalid number"')
          done()
        })
    })
    ...
    test('Convert an invalid number AND unit such as 3/7.2/4kilomegagram: GET request to /api/convert.', (done) => {
      chai
        ...
        .end( (err, res) => {
          assert.equal(res.body, 'invalid number and unit', 'returned text should be "invalid number and unit"')
          done()
        })
    })
    ...
    test('Convert with no number such as kg: GET request to /api/convert.', (done) => {
      chai
        ...
        .end( (err, res) => {
          ...
          assert.approximately(res.body.returnNum, 2.20462, 0.1, 'returnNum should be "2.20462"')
